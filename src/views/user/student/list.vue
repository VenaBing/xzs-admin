<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%">
      <el-table-column prop="stuId" label="Id" />
      <el-table-column prop="accountNo" label="用户名"/>
      <el-table-column prop="stuName" label="真实姓名" />
      <el-table-column prop="gradeId" label="学级" :formatter="levelFormatter"/>
      ...
      <el-table-column width="270px" label="操作" align="center">
        <template slot-scope="{row}">
          <el-button size="mini"  @click="changeStatus(row)" class="link-left">
            {{ statusBtnFormatter(row.stuStatus) }}
          </el-button>
          <router-link :to="{path:'/user/student/edit', query:{id:row.stuId}}" class="link-left">
...
export default {
  ...
  methods: {
    search (e) {
      ...
      userApi.queryStudentList(this.queryParam).then(data => {
        const re = data.data
    ...
    changeStatus (row) {
      let _this = this
      userApi.modifyStudentInfo({ stuId: row.stuId, stuStatus: row.stuStatus === 1 ? 2 : 1 }).then(re => {
    ...
    deleteUser (row) {
      ...
      this.$confirm('确认删除?', '提示', {
        ...
        userApi.delStudentInfo({ stuId: row.stuId }).then(re => {
          if (re.resultCode === 10000) {
            _this.search()
            _this.$message.success(re.resultMsg)
          } else {
