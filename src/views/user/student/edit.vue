  ...
  <div class="app-container">
    <el-form :model="form" ref="form" label-width="100px" v-loading="formLoading" :rules="rules">
      ...
      <el-form-item label="状态：" required>
        <el-select v-model="form.stuStatus" placeholder="状态">
...
export default {
  ...
  created () {
    ...
    if (id && parseInt(id) !== 0) {
      ...
      userApi.queryStudentList({ stuId: id }).then(re => {
    ...
    submitForm () {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (id) {
            this.form.stuId = id
            if (!this.form.accountPass) {
              delete this.form.accountPass
            }
            userApi.modifyStudentInfo(this.form).then(data => {
            ...
            userApi.addStudentInfo(this.form).then(data => {
              if (data.resultCode === 10000) {
                _this.$message.success(data.resultMsg)
                _this.delCurrentView(_this).then(() => {
                  _this.$router.push('/user/student/list')
